...
const Examples = ({onClickRecommendation} : Props) => {
  ...
  <section className='examples'>
    <div className='examples_inner'>
      ...
      {matches && <div className='examples_grid'>
        {examples.map((example)=><Example key={example.songId} song={example} onClickRecommendation={onClickRecommendation}/>)}
      </div>}
      {!matches && <div className='examples_grid_small_screen'>
        {examples.splice(2).map((example)=><Example key={example.songId} song={example} onClickRecommendation={onClickRecommendation}/>)}
      </div>}
    </div>
  </section>)
}
